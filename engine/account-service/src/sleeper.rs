...
impl SleeperClient {
    ...
    pub async fn get_user_leagues(&self, user_id: &str, season: &str) -> Result<Vec<LeagueOption>, AccountServiceError> {
        ...
        println!("DEBUG: Found {} leagues for user {} in season {}", leagues_array.len(), user_id, season);
        ...
        for (i, league_json) in leagues_array.iter().enumerate() {
            ...
            let league_id = league_json.get("league_id")
                .and_then(|v| v.as_str())
                .ok_or_else(|| AccountServiceError::SleeperApiError {
                    message: format!("Missing league_id in league {}", i),
                })?;
            
            let league_name = league_json.get("name")
                .and_then(|v| v.as_str())
                .unwrap_or("Unnamed League");
            
            let league_season = league_json.get("season")
                .and_then(|v| v.as_str())
                .unwrap_or(season);
            ...
            if rosters_response.status().is_success() {
                ...
                let rosters_array: Vec<serde_json::Value> = serde_json::from_str(&rosters_text)
                    .map_err(|e| AccountServiceError::SleeperApiError {
                        message: format!("Failed to parse rosters: {} - Response: {}", e, rosters_text),
                    })?;
                
                // Find user's roster
                for roster_json in rosters_array {
                    let owner_id = roster_json.get("owner_id")
                        .and_then(|v| v.as_str());
                    
                    if owner_id == Some(user_id) {
                        let roster_id = roster_json.get("roster_id")
                            .and_then(|v| v.as_u64())
                            .ok_or_else(|| AccountServiceError::SleeperApiError {
                                message: format!("Missing roster_id for user in league {}", league_id),
                            })?;
                ...
                println!("DEBUG: Failed to get rosters for league {}: {}", league_id, rosters_response.status());
            }
        }
    ...
    pub async fn get_season_points(&self, league_id: &str, roster_id: &str) -> Result<u32, AccountServiceError> {
        ...
        // Parse as JSON array
        let rosters_array: Vec<serde_json::Value> = serde_json::from_str(&response_text)
            .map_err(|e| AccountServiceError::SleeperApiError {
                message: format!("Failed to parse rosters JSON: {} - Response: {}", e, response_text),
            })?;
        
        // Parse roster_id from string to u32 for comparison
        let roster_id_num = roster_id.parse::<u32>()
            .map_err(|_| AccountServiceError::SleeperApiError {
                message: format!("Invalid roster_id: {}", roster_id),
            })?;
        
        // Find the specific roster and get its fantasy points
        for roster_json in rosters_array {
            let roster_id_from_json = roster_json.get("roster_id")
                .and_then(|v| v.as_u64())
                .unwrap_or(0) as u32;
            ...
            if roster_id_from_json == roster_id_num {
                ...
                if let Some(settings) = roster_json.get("settings") {
                    let fpts = settings.get("fpts")
                        .and_then(|v| v.as_u64())
                        .unwrap_or(0) as u32;
                    
                    let fpts_decimal = settings.get("fpts_decimal")
                        .and_then(|v| v.as_u64())
                        .unwrap_or(0) as u32;
                    ...
                    println!("DEBUG: Fantasy points - fpts: {}, fpts_decimal: {}, total: {}", fpts, fpts_decimal, total_points);
    ...
    pub async fn get_nfl_state(&self) -> Result<NflState, AccountServiceError> {
        ...
        let state: NflState = serde_json::from_str(&response_text)
            .map_err(|e| AccountServiceError::SleeperApiError {
                message: format!("Failed to parse NFL state JSON: {} - Response: {}", e, response_text),
            })?;
        
        println!("DEBUG: Parsed NFL state - season: {}, league_season: {}", state.season, state.league_season);
        Ok(state)
    }
