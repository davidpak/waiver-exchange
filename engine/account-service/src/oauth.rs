...
impl GoogleOAuthClient {
    ...
    pub fn new(config: &OAuthConfig) -> Result<Self, AccountServiceError> {
        let client = BasicClient::new(
            ...
            AuthUrl::new("https://accounts.google.com/o/oauth2/auth".to_string()).map_err(|e| {
                AccountServiceError::InvalidConfig { message: format!("Invalid auth URL: {}", e) }
            })?,
            Some(TokenUrl::new("https://oauth2.googleapis.com/token".to_string()).map_err(
                |e| AccountServiceError::InvalidConfig {
                    message: format!("Invalid token URL: {}", e),
                },
            )?),
        )
        .set_redirect_uri(RedirectUrl::new(config.redirect_url.clone()).map_err(|e| {
            AccountServiceError::InvalidConfig { message: format!("Invalid redirect URL: {}", e) }
        })?);
    ...
    pub async fn get_user_info(
        &self,
        access_token: &str,
    ) -> Result<GoogleUserInfo, AccountServiceError> {
