...
pub async fn init_analytics(config: AnalyticsConfig) -> Result<AnalyticsEngine, anyhow::Error> {
    ...
    Ok(AnalyticsEngine { storage, aggregator, ingestion, query_engine, config })
}
...
impl AnalyticsEngine {
    ...
    pub async fn start(self) -> Result<(), anyhow::Error> {
        ...
        let ingestion_handle = tokio::spawn(async move { self.ingestion.start().await });

        // Start aggregation service
        let aggregator_handle = tokio::spawn(async move { self.aggregator.start().await });

        // Start query service
        let query_handle = tokio::spawn(async move { self.query_engine.start().await });

        // Wait for all services
        let _ = tokio::try_join!(ingestion_handle, aggregator_handle, query_handle)?;
