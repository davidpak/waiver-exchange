...
impl MetricsAggregator {
    ...
    pub fn new(config: AggregationConfig) -> Self {
        Self { config, buffers: Arc::new(Mutex::new(HashMap::new())) }
    }
    ...
    async fn flush_buffer(
        ...
        tracing::debug!(
            "Flushing {} events, aggregated to {} events",
            buffer.events.len(),
    ...
    fn aggregate_performance_events(events: &[&AnalyticsEvent]) -> Result<AnalyticsEvent> {
        ...
        for event in events {
            if let Some(crate::analytics::analytics_event::Data::Performance(perf)) = &event.data {
    ...
    fn aggregate_business_events(events: &[&AnalyticsEvent]) -> Result<AnalyticsEvent> {
        ...
        for event in events {
            if let Some(crate::analytics::analytics_event::Data::Business(biz)) = &event.data {
    ...
    fn aggregate_health_events(events: &[&AnalyticsEvent]) -> Result<AnalyticsEvent> {
        ...
        for event in events {
            if let Some(crate::analytics::analytics_event::Data::Health(health)) = &event.data {
    ...
    fn aggregate_operational_events(events: &[&AnalyticsEvent]) -> Result<AnalyticsEvent> {
        ...
        for event in events {
            if let Some(crate::analytics::analytics_event::Data::Operational(op)) = &event.data {
