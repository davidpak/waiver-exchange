...
pub enum CfgError {
    ...
    BatchZero,
    ArenaZero,
    ExecShiftTooSmallOrLarge,
}

impl EngineCfg {
    pub fn validate(&self) -> Result<(), CfgError> {
        if self.price_domain.tick == 0 {
            return Err(CfgError::InvalidTick);
        }
        if self.price_domain.ceil < self.price_domain.floor {
            return Err(CfgError::DomainEmpty);
        }
        if self.batch_max == 0 {
            return Err(CfgError::BatchZero);
        }
        if self.arena_capacity == 0 {
            return Err(CfgError::ArenaZero);
        }
        if self.exec_shift_bits == 0 || self.exec_shift_bits > 32 {
            return Err(CfgError::ExecShiftTooSmallOrLarge);
        }
        Ok(())
    }
}
