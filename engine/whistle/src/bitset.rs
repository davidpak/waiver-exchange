...
impl Bitset {
    ...
    #[inline] pub fn len_bits(&self) -> usize { self.len_bits }
    #[inline] fn check(&self, i: usize) { assert!(i < self.len_bits, "bit index {i} OOB"); }
    #[inline] pub fn is_empty(&self) -> bool { self.words.iter().all(|&w| w == 0) }

    #[inline] pub fn set(&mut self, i: usize) {
        self.check(i); let w = i >> 6; let b = i & 63; self.words[w] |= 1u64 << b;
    }
    #[inline] pub fn clear(&mut self, i: usize) {
        self.check(i); let w = i >> 6; let b = i & 63; self.words[w] &= !(1u64 << b);
    }
    #[inline] pub fn get(&self, i: usize) -> bool {
        self.check(i); let w = i >> 6; let b = i & 63; ((self.words[w] >> b) & 1) != 0
    }

    /// Next set bit at or after `i`.
    pub fn next_one_at_or_after(&self, i: usize) -> Option<usize> {
        if i >= self.len_bits { return None; }
        let mut w = i >> 6; let off = i & 63;
    ...
    pub fn prev_one_at_or_before(&self, i: usize) -> Option<usize> {
        if self.len_bits == 0 { return None; }
        let t = if i >= self.len_bits { self.len_bits - 1 } else { i };
        let mut w = t >> 6; let off = t & 63;
...
mod tests {
    ...
    fn set_get_clear_across_boundaries() {
        ...
        assert!(bs.is_empty());
        for &i in &[0usize, 63, 64, 65, 127, 128, 129] { bs.set(i); assert!(bs.get(i)); }
        for &i in &[0usize, 63, 64, 65, 127, 128, 129] { bs.clear(i); assert!(!bs.get(i)); }
        assert!(bs.is_empty());
    }
    ...
    fn next_one_at_or_after_works() {
        let mut bs = Bitset::with_len(130);
        for &i in &[3usize, 63, 64, 90, 129] { bs.set(i); }
