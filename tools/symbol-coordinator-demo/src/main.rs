...
use order_router::{
    InboundMsgWithSymbol, OrderRouter, RouterConfig, SymbolCoordinatorApi as OrderRouterApi,
};
use std::sync::Arc;
use std::thread;
...
fn main() {
    ...
    // Create ExecutionManager for the demo
    let execution_manager =
        Arc::new(ExecutionManager::new(execution_manager::ExecManagerConfig::default()));
    let coordinator = SymbolCoordinator::new(coord_config.clone(), execution_manager);
    ...
    let coordinator_adapter = Box::new(CoordinatorAdapter { coordinator: coordinator_arc.clone() });
    ...
    for (symbol_id, player_name, orders) in order_scenarios {
        ...
        for (i, (side, price, qty, description)) in orders.iter().enumerate() {
            ...
            match result {
                ...
                Err(e) => println!(
                    "    ⚠️  {side_str} @ {price} ({qty} units) - {description} (expected: {e:?})"
                ),
            }
...
impl OrderRouterApi for CoordinatorAdapter {
    fn ensure_active(
        ...
        coordinator_guard
            ...
            .map(|ready_at| order_router::ReadyAtTick {
                next_tick: ready_at.next_tick,
                queue_writer: order_router::OrderQueueWriter { queue: ready_at.queue_writer.queue },
            })
